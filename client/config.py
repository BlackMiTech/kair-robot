...
def init(config_name='profile.yml'):
    ...
    if not os.path.exists(kairpath.CONFIG_PATH):
        try:
            os.makedirs(kairpath.CONFIG_PATH)
        except OSError:
            _logger.error("Could not create config dir: '%s'",
                          kairpath.CONFIG_PATH, exc_info=True)
            raise

    # Check if config dir is writable
    if not os.access(kairpath.CONFIG_PATH, os.W_OK):
        _logger.critical("Config dir %s is not writable. Dingdang " +
                         "won't work correctly.",
                         kairpath.CONFIG_PATH)

    config_file = kairpath.config(config_name)
    global _config
