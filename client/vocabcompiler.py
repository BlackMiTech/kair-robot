...
class JuliusVocabulary(AbstractVocabulary):
    ...
    def _compile_vocabulary(self, phrases):
        ...
        lexicon_file = kairpath.data('julius-stt', 'VoxForge.tgz')
        lexicon_archive_member = 'VoxForge/VoxForgeDict'
        profile_path = kairpath.config('profile.yml')
...
def get_keyword_phrases():
    """
    Gets the keyword phrases from the keywords file in the kair data dir.
    ...
    with open(kairpath.data('keyword_phrases'), mode="r") as f:
        for line in f:
            phrase = line.strip()
            if phrase:
