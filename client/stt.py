...
class BaiduSTT(AbstractSTTEngine):
    ...
    def get_token(self):
        cache = open(os.path.join(kairpath.TEMP_PATH, 'baidustt.ini'),
                     'a+')
        try:
            pms = cache.readlines()
            if len(pms) > 0:
...
class SnowboySTT(AbstractSTTEngine):
    ...
    Snowboy STT 离线识别引擎（只适用于离线唤醒）
        ...
        snowboy:
            model: '/home/pi/.kair/snowboy/kairkair.pmdl'  # 唤醒词模型
            sensitivity: "0.5"  # 敏感度
        ...
    """
    ...
    def __init__(self, sensitivity, model, hotword, **kwargs):
        ...
        self.model = model
        self.resource_file = os.path.join(kairpath.LIB_PATH,
                                          'snowboy/common.res')
        try:
    ...
    def get_config(cls):
        ...
        if 'model' not in profile:
            profile['model'] = os.path.join(
                kairpath.LIB_PATH, 'snowboy/kair.pmdl')
