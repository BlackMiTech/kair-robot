...
# Add kairpath.LIB_PATH to sys.path
sys.path.append(kairpath.LIB_PATH)

parser = argparse.ArgumentParser(description='kair Voice Control Center')
...
class kair(object):
    ...
    def run(self):
        ...
        persona = config.get("robot_name", 'kair')
...
*******************************************************"
*             kair - 中文语音对话机器人               *
*          (c) 2017 kair <jimmy@blademiservice.com>             *
*******************************************************

如需查看log，可以执行 `tail -f kair所在目录/temp/kair.log`
...
''')
    ...
    if not args.no_network_check and not diagnose.check_network_connection():
        logger.warning("Network not connected. This may prevent kair " +
    ...
    try:
        ...
        logger.info("kair get Keyboard Interrupt, exit.")
        print("kair exit.")
    except Exception:
        logger.exception("kair quit unexpectedly!")
        if not args.verbose:
            msg = traceback.format_exc()
            print("** kair quit unexpectedly! ** ")
            print(msg)
        sys.exit(1)
